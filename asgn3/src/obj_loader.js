// Matsuda WebGL CH 10 -> taken from https://people.ucsc.edu/~jbrowne2/asgn3/World.html
...
class OBJ {
    constructor() {
        this.drawing_info = null;
        this.vertex_buffer = null;
        this.normal_buffer = null;
        this.index_buffer = null;
        this.color = [1.0,0.9,0.0,1.0];
        this.type = 'obj';
        this.model_matrix = new Matrix4();
        this.normal_matrix = new Matrix4();
        this.objDoc = null;
        this.url = '../img/Little_Fox/model.obj';
        this.scale = 2.0;
        this.texture_map = 0;
    }

    onReadOBJFile(fileString) {
        var objDoc = new OBJDoc(this.url);
        var result = objDoc.parse(fileString, this.scale, true);
        if (!result) {
            this.objDoc = null; this.drawing_info = null;
            console.log("OBJ file parsing error.");
            return;
        }
        this.objDoc = objDoc;
        this.drawing_info = objDoc.getDrawingInfo();
    }
    ...
    readOBJFile() {
        ...
        request.onreadystatechange = () => {
            if (request.readyState == 4 && request.status != 404) {
                this.onReadOBJFile(request.responseText);
            }
        }
    ...
    render() {
        if(this.objDoc != null) {
            this.drawing_info = this.objDoc.getDrawingInfo();
            this.objDoc = null;
        }
        if(this.drawing_info == null) {
            return;
        }

        gl.uniform4f(u_FragColor, this.color[0], this.color[1], this.color[2], this.color[3]);
        gl.uniform1i(u_whichTexture, this.texture_map);
        ...
        gl.bufferData(gl.ARRAY_BUFFER, (this.drawing_info.vertices), gl.STATIC_DRAW);
        ...
        // gl.bufferData(gl.ARRAY_BUFFER, (this.drawing_info.normals), gl.STATIC_DRAW);
        ...
        gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, (this.drawing_info.indices), gl.STATIC_DRAW);

        gl.frontFace(gl.CCW);
        gl.drawElements(gl.TRIANGLES, this.drawing_info.indices.length, gl.UNSIGNED_SHORT, 0);
        gl.frontFace(gl.CW);
    }
